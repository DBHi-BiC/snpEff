/*=======================================================================
 MATALIN. (C) Copyright 1992 Fred Hutchinson Cancer Research Center.
      matalin.c reads a file containing a scoring matrix and
      outputs an alin matrix.  Assumes alphabet for file is
...
/*-------If some entries are still missing, assume symmetry ---------*/
 printf(">A  R  N  D  C  Q  E  G  H  I  L  K  M  F  P  S  T  W  Y  V  B  Z  X\n");
   for (row=0; row<AAS; row++)
   {
	...
	if ((row==22 || col==22) && scores[row][col]==-999)
	   scores[row][col] = -1;		/* X -> -1 */
	printf("%2d ", scores[row][col]+8);
     }
    printf(" %1s\n", ctemp);
   }
/*   printf("   A  R  N  D  C  Q  E  G  H  I  L  K  M  F  P  S  T  W  Y  V  B  Z  X\n");*/
